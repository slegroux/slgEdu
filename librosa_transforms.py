...
class MFCC(object):
    ...
    def __call__(self, sample):
        ...
        mfcc = librosa.feature.mfcc(y=audio, sr=self.sr, hop_length=self.hop_length, n_mfcc=self.n_mfcc)
...
class MelSpectrogram(object):
    ...
    def __call__(self, sample):
        ...
        audio, label = sample['X'], sample['y']
        audio = audio.numpy()
        melspec = librosa.feature.melspectrogram(y=audio, sr=self.sr, hop_length=self.hop_length, n_mels=self.n_mels)
...
class LogPowerSpectrogram(object):
    ...
    def __call__(self, sample):
        ...
        logspec = torch.from_numpy(logspec)
...
class PadTrim(object):
    ...
    def __call__(self, sample):
        ...
        sample = {'X': torch.from_numpy(audio), 'y': label}
        return sample
